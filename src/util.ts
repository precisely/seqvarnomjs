...
function accession(input: SVNArguments): string {
...
function head(input: SVNArguments): string {
  return accession(input) + ':g.';
}

function wildType(input: SVNArguments): string {
  return `[${input.start}=]`;
}

function substitution(input: SVNArguments, altBaseIndex: number): string {
  return `[${input.start}${input.ref.toUpperCase()}>${input.altBases[altBaseIndex].toUpperCase()}]`;
}

function hgvsSingleSubstitution(input: SVNArguments): string {
...
export function makeSVN(input: SVNArguments): string {
  if (input.altBases.length === 1) {
    ...
    throw new Error(`Unexpected inputs to makeSVN zygosity: ${input.zygosity}, altBases: ${input.altBases}`);
  }
}
